...
// plateau phase uses Factor 0, which delay() special-cases to always return
// the same duration unchanged -- so it never exercises the cap-crossing
// branch and Steps merely counts down from a very large number, for all
// practical purposes never exhausting.
